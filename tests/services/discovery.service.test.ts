...
import type { DiscoveryCacheRepository } from "../../src/repositories/discovery-cache.repository";
import type { FsScannerRepository } from "../../src/repositories/fs-scanner.repository";
import {
  createDiscoveryService,
  DEFAULT_SIBLING_DEPTH,
...
describe("discovery.service cache integration", () => {
  ...
  test("loadCached() returns null when discovery config slice changed", async () => {
    ...
    const next = await svc.loadCached(makeConfig({ include: [".bashrc"], auto_track: false }));
  ...
  test("commitAccept() removes the path from the cached snapshot", async () => {
    ...
    expect(r.ok).toBe(true);
    expect(removed).toEqual([{ hash: removed[0]!.hash, path: "/h/.config/fish/config.fish" }]);
    ...
    const svc = createDiscoveryService({
      scanner: {
        ...
        async siblings(): Promise<
          Result<readonly string[], import("../../src/repositories/types").RepoError>
        > {
